...
requires_init = not os.path.exists(DB_PATH)

log("Establishing connection.", "DB_INIT", level=logging.DEBUG)
conn = sq.connect(DB_PATH, timeout=20, isolation_level=None)
...
# Initialise the database if it was just created
if requires_init:
    log("Running first-time setup.", "DB_INIT")
    # Execute schema file
    with conn:
        with open(SCHEMA_PATH, 'r') as script:
...
with conn:
    ...
    if not version_exists:
        # Create version table and insert version 0
        cursor.execute('CREATE TABLE VersionHistory (version INTEGER NOT NULL, time INTEGER NOT NULL)')
        now = datetime.timestamp(datetime.utcnow())
        cursor.execute('INSERT INTO VersionHistory VALUES (0, {})'.format(datetime.timestamp(datetime.utcnow())))
    ...
    if current_version != REQUIRED_VERSION:
        ...
        raise Exception(
            ("Database version is {}, required version is {}. "
             "Please migrate database.").format(current_version, REQUIRED_VERSION)
        )
